...
with tab1:
    ...
    sel_date1 = col2.date_input(
        "Date", key="tp2", value=today, format="YYYY-MM-DD", max_value=today
    )
    ...
    with st.expander(f"GFS Situation"):
        ...
        url1, url2, url3, url4 = load_GFS_analysis(
            select_date=str(sel_date1), place=sel_place
        )
        ...
        url5, url6, url7, url8 = load_crop_analysis(
            select_date=str(sel_date1), place=sel_place
        )
    ...
    with st.expander(f"GFS Analysis"):
        ...
        sel_zoom = col3.selectbox(
            "Zoom favorite", key="co3", options=file_data.keys(), index=None
        )
        if sel_zoom:
        ...
        img1 = add_rectangle(
            ...
            left=int(x_values),
            top=int(y_values),
            right=int(x_values + w_values),
            bottom=int(y_values + h_values),
        )

        col1a, col2a, col3a = col1.columns([6, 0.2, 3.8])
        col1a.image(img1)
        validate2 = col3.button(
            "Analyse", key="co5", use_container_width=True, type="primary"
        )
        col3.write("")
        ...
        if validate2 and len(date_serie) != 0:
            ...
            for d in date_serie:
                try:
                    url1s, url2s, url3s, url4s = load_GFS_analysis(
                        select_date=str(d), place=sel_place
                    )
                    im_crop, df = crop_image(
                        url=url1s,
                        left=int(x_values),
                        top=int(y_values),
                        right=int(x_values + w_values),
    ...
    with st.expander(f"Crop calendar"):
        ...
        select = select[
            (select["REGION"].isin([sel_place]))
            & (select["PRODUCTION"].isin([sel_product.upper()]))
        ].copy()

        def color_vowel(value):
            color = (
                f"background-color: #6B8C4A; color: #2E4022"
                if value in ["PLA"]
                else (
                    f"background-color: #D9B343; color: #8C7F3E;"
                    if value in ["HAR"]
                    else None
                )
            )
        ...
        st.dataframe(
            select_df.style.map(color_vowel, subset=list_col),
            hide_index=True,
        ...
        file_path = os.path.join(
            directory, f"../reports/figures/{short_month[:3]}_calendar.gif"
        )
        st.write("")
        st.write("")
