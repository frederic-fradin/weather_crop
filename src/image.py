...
color_excluded = ['rgb(200,200,200)', 'rgb(255,255,255)', 'rgb(72,108,139)',
                  'rgb(51,51,0)', 'rgb(167,167,0)']
...
def create_chart(df):
    ...
    pvt1 = pd.pivot_table(data=df[(df['percent'] !=0) & (df['date'] == df['date'].max())].copy(), aggfunc='sum',
                          index='colorCode', values='percent', fill_value=0).reset_index()
    fig1 = px.bar(pvt1, x='colorCode', y='percent', color='colorCode', text='percent',
                color_discrete_sequence=pvt1['colorCode'], width=1080, height=480, text_auto=True)
    ...
    pvt2 = pd.pivot_table(data=df[df['score'] !=0], aggfunc='sum',
                          index='date', values='score', fill_value=0).reset_index()
...
def crop_image(url:str, left:int, top:int, right:int, bottom:int, date:date, place:str) -> object:
    ...
    df['colorCode'] = df.apply(lambda x: 'rgb(' + str(x['Red']) +',' + str(x['Green']) +',' + str(x['Blue']) +')', axis=1)
    df['pixels'] = pixel_count
    df['percent'] = df.apply(lambda row: round(row['Value'] / row['pixels'],4) * 100 if row['pixels'] !=0 else 0, axis=1)
